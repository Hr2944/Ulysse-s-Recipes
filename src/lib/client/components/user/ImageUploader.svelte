...
<div class="w-full">
	<label
		class="relative flex h-64 w-full cursor-pointer items-center justify-center rounded-3xl border-2 border-dashed border-primary/50 bg-primary/5 text-center transition-all duration-300 hover:border-primary hover:bg-primary/10 hover:scale-[1.01]"
	>
		{#if previewUrl}
			<img src={previewUrl} alt="Présentation de la recette" class="h-full w-full rounded-3xl object-cover" />
			<div
				class="absolute inset-0 flex flex-col items-center justify-center rounded-3xl bg-black/50 text-white opacity-0 transition-opacity hover:opacity-100">
