...
<article class="relative min-h-screen bg-surface pb-20 sm:pb-0">
			...
			<ol class="relative border-l-2 border-primary/10 ml-3 md:ml-4 space-y-8">
				{#each sortedSteps as step (step.step_number)}
					<li class="relative pl-8 md:pl-12 duration-500">
						...
						<div
							class="flex flex-col gap-3 rounded-3xl p-6 bg-white shadow-sm shadow-primary/5 border border-primary/5 hover:shadow-md transition-all duration-300 {checkedSteps.has(step.step_number) ? 'opacity-60' : ''}">
							...
							<p
								class="text-lg text-on-surface/90 leading-relaxed {checkedSteps.has(step.step_number) ? 'line-through' : ''}">
			...
			<div class="flex h-full flex-col px-6">
				<div class="sticky top-0 pt-6 bg-surface z-10">
					<div class="flex items-center justify-between pb-4">
						...
						<button aria-label="Fermer la liste des ingrédients" onclick={() => (isIngredientsSheetOpen = false)}
										class="text-on-surface/80 rotate-45 hover:bg-on-surface/20 rounded-full p-1 transition-colors">
				...
				<div
					...
					<button aria-label="Enlever un couvert" onclick={() => updateServings(servings - 1)} disabled={servings <= 1}
									class="rounded-full bg-primary/5 p-3 text-primary hover:bg-primary hover:text-white transition-colors disabled:opacity-50">
					...
					<button aria-label="Ajouter un couvert" onclick={() => updateServings(servings + 1)}
									class="rounded-full bg-primary/5 p-3 text-primary hover:bg-primary hover:text-white transition-colors">
						<svg height="20" viewBox="0 -960 960 960" width="20" fill="currentColor">
							<path d={plus} />
						</svg>
					</button>
	...
	<button
		class="fixed z-10 bottom-20 sm:bottom-6 right-4 sm:right-6 pointer-events-auto flex items-center justify-center gap-2 rounded-full bg-primary px-8 py-4 font-bold text-on-primary shadow-lg transition-transform hover:scale-105 active:scale-100"
